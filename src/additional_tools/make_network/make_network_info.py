...
os.system("python $AP_PLN_HOME/src/additional_tools/make_network/get_info_network.py %s %s %s %s > /dev/null" % (f_out_subnet,f_list,f_dir,f_out_infoLinks))

# Plot the subnetwork 
logger.info("3) Plot the subnetwork")
os.system("Rscript $AP_PLN_HOME/src/scripts_R/make_network.R %s %s %s > /dev/null" % (f_out_infoLinks,dir_work,out_suffixe))
